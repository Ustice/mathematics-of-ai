...
The layer's forward pass computes \(Wx+b\) and stores the input. Its backward pass receives \(\bar z\), then computes the three gradients above.
...
5. Let \(z=w^Tx+b\) and \(a=\sigma(z)\). Derive \(\nabla_w a\), \(\nabla_x a\), and \(\partial a/\partial b\) using the chain rule.
...
- For a scalar loss, the linear-layer gradients are \(W^T\bar z\), \(\bar z x^T\), and \(\bar z\).
