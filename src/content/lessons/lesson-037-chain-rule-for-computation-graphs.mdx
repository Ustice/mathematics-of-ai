...
1. Compute the forward values $u$ and $L$.
2. Write the local derivatives $\partial L/\partial u$ and $\partial u/\partial x$.
3. Compute $dL/dx$ using the stored value of $u$.
4. In one sentence, explain why replacing $u^2$ with $x^2$ would change the computation.
...
write $J_F(x,y)$ by differentiating one output component at a time. Label rows by outputs and columns by inputs before filling in the entries.
...
At $z=3$, run the backward pass from $\bar L=1$. Show $\bar a$, $\bar b$, the contribution from each child to $\bar z$, and their accumulated total.

Then write the general rule:

```math
\bar z=	ext{contribution through }a+	ext{contribution through }b.
```

using $\bar a$, $\bar b$, $da/dz$, and $db/dz$.
...
1. Compute the forward value $F(2,1)$.
2. Write $J_F(x,y)$ and $J_G(u,v)$ with shapes.
3. Evaluate both Jacobians at the correct local values.
4. Compute $J_{G\circ F}(2,1)=J_G(F(2,1))J_F(2,1)$.
5. Convert the resulting scalar-output Jacobian into a column gradient.
6. Check the result by expanding $G(F(x,y))$ only after completing the graph-based calculation.
...
After Practice 37.1 is secure, Lesson 38 will make matrix-calculus layout conventions explicit. We will use shape checking to distinguish gradients with respect to vectors and matrices, translate between common notation conventions, and derive reusable identities without relying on memorized formula tables.
